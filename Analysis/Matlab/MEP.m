...
%         'threshold': use same criteria as threshold (below)
...
function [mep, options] = MEP(data, samplehz, pulsetime, options)
    ...
    if isfield(options, 'window') && ~isempty(options.window)
        if ~ismatrix(options.window)
            error('options.window has too many dimensions, should be 2x1 or 3x1 array');
        end
	...
	switch options.auc
	        ...
	        % not coded yet
            case 'window'
                mep.auc(n) = sum(abs(sweep(start:finish, n))) ./ (samplehz./1000);% use the same MEP window & measure sum of absolute voltages divided by timepoints per ms
        end
